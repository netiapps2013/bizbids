...
class UserController extends Controller
{
	public function authenticateAction(Request $request, $email, $enquiryid)
	{
		...
		if($request->isMethod('POST')){
			if($form->isValid()){
				...
				return $this->render('BBidsBBidsHomeBundle:User:authenticate.html.twig', array('form'=>$form->createView()));
			}
		}
				...
				if(isset($id)){
					$user = new User();
                                        ...
                                        $message = \Swift_Message::newInstance()
                                                ...
                                                ->setBody($this->render('BBidsBBidsHomeBundle:Home:activationemail.html.twig', array('userid'=>$id, 'userhash'=>$userhash)));
	...
	public function vendorenquiriesAction()
	{
		...
		$query = $em->createQueryBuilder()
                         ->select(array('e.subj, e.description, e.city, c.category, e.created, e.id, v.leadstatus '))
			 ->from('BBidsBBidsHomeBundle:Enquiry','e')
		...
		if($request->isMethod('POST')){
				...
				$categorycount = count($form['category']->getData());
                                ...
                                $citycount = count($form['city']->getData());
								                                           ...
								                                           /*	print_r($form['segment']->getData());
				print_r($form['category']->getData());
				print_r($form['subcategory']->getData());
				print_r($form['city']->getData()); */
